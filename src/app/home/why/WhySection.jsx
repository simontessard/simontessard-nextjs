import Uptitle from "@/app/common/utils/Uptitle";
import AnimText from "@/app/common/utils/AnimText";
import AnimParagraph from "@/app/common/utils/AnimParagraph";
import TwoColumns from "@/app/home/why/TwoColumns";
import CTABase from "@/app/common/ui/CTABase";
import Image from "next/image";

export default function WhySection() {
    return (
        <section id="why" style={{backgroundImage: "url(/cta.png)"}}
            className="relative bg-cover bg-center overflow-hidden py-14 md:py-16 xl:py-22 2xl:py-24">

            <Image width={1000} height={1000} src="/logo.png" alt="Logo Simon TESSARD"
                   className="absolute top-10 md:top-0 left-0 right-0 bottom-0 m-auto size-200 md:size-250 xl:size-325 opacity-3 md:opacity-2.5 object-contain"/>

            <Uptitle className="text-white"
                     useScrollTrigger>Freelance vs Agence</Uptitle>

            <AnimText
                useScrollTrigger
                tag="h2"
                className="css-title-section text-white container mb-4 md:mb-5 xl:mb-6"
            >
                Pourquoi <span className="css-word-italic"> choisir </span>
                <br/>
                un <span className="css-word-italic"> freelance </span>?
            </AnimText>

            <AnimParagraph
                useScrollTrigger
                tag="p"
                className="css-subtitle-section container text-white md:max-w-3xl mb-12 md:mb-16 2xl:mb-18">
                Voici quelques avantages concrets à collaborer avec moi
                <br className="max-md:hidden"/> plutôt qu’avec une agence.
            </AnimParagraph>

            <TwoColumns/>

            <CTABase
                useScrollTrigger
                variant="white"
                to="/offres"
                text="Découvrir mes offres"
                customClass="mx-auto mt-6 md:mt-10"
            />

        </section>
    );
}